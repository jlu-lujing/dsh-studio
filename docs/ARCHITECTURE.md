...
- **形态**：DSH **agent preset**（`~/.dsh/.agent-presets/<id>`），不是 Cordis bundle；
- **接入**：`packages/dsh-studio/preset/` 下每模式一个子目录（`boost-mode/`、`jspace-mode/`），`src/preset.ts` 按 id 导入/删除（幂等、非破坏；目标已存在不覆盖；staging + rename 原子落位）；
- **开关**：store 里每个模式一个功能（`dsh-boost-mode` / `dsh-jspace-mode`），installable 且不可 toggle；启用即自动导入对应 preset，删除即禁用。

#### 6.2.1 Boost Mode（满血模式）

- **来源**：基于 [dsh-anchored-standard](https://github.com/xiaobright/dsh-anchored-standard) 二阶段 preset；安装到 `~/.dsh/.agent-presets/boost-mode`；
- **名称**：DSH 预设选择器显示 `Boost Mode`（中文「满血模式」）。

#### 6.2.2 J-Space Mode（标准模式 + J-Space）

- **形态**：基于 dsh-anchored-standard 二阶段引导，并在 preset 内联 **J-Space Cognitive Suite V3.6**（`preset/jspace-mode/j-space/`）；
- **发现**：`preset/jspace-mode/inline-jspace.mjs` 用 `ctx.skills.registerProvider` 把内联 j-space 注册进 skills，使 `skill_search` / `skill_load` 可发现，不触碰 `~/.dsh/skills`；
- **persona**：引导模型在深度推理/长任务/工具重任务/验证恢复时 `skill_load j-space` 按需加载；
- **安装**：`~/.dsh/.agent-presets/jspace-mode`，默认关闭，功能商店一键安装/删除。
