...
} from '../src/webui/client/themes.ts'
...
test('localStorage 损坏/空数据回退为安全默认', () => {
  const mem = mockStorage()
  mem.set('dsh-studio-webui.themes.v1', '{broken json')
  assert.deepEqual(loadStored(), { custom: [], active: null, global: {} })
  mem.set('dsh-studio-webui.themes.v1', JSON.stringify({ custom: [null], active: 3, global: { x: 'bad' } }))
  assert.deepEqual(loadStored(), { custom: [], active: null, global: {} })
})
