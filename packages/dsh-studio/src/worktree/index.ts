/**
 * dsh-studio-worktree host plugin.
 *
 * host 侧插件：不声明 dsh.bundle，行由 dsh-studio 聚合包挂载（与 scheduler /
 * notifier 一致）。提供一组 loopback 管理路由，把 `git worktree` 封装成
 * 「列表 / 新建 / 删除 / 清理 / 会话归属判定」：
 *
 *   GET  /dsh-studio-worktree/worktrees?cwd=<path>   → { root, defaultParent, worktrees }
 *   POST /dsh-studio-worktree/worktrees              body { branch, path?, base?, cwd? } → 新建
 *   POST /dsh-studio-worktree/worktrees/remove       body { path, force?, cwd? }         → 删除
 *   POST /dsh-studio-worktree/worktrees/prune?cwd=   → 清理失效登记
 *   GET  /dsh-studio-worktree/status?cwd=<path>      → 目录是否 git 仓库
 *   GET  /dsh-studio-worktree/attribution?cwd=<path> → 会话目录归属 main/worktree
 *
 * `cwd` 缺省时退回 dsh 进程工作目录（兼容最早的 host-only 用法）。
 *
 * 路由挂在 loopback DSH webServer（生命周期同 scheduler）：局域网通过
 * lan-auth 网关进来的流量在 DSH 眼里仍是 loopback 调用，因此与其它 dsh-studio
...
/** Cordis plugin name. */
export const name = 'dsh-studio-worktree'
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.effect(() => () => dispose?.(), 'dsh-studio-worktree.http-routes')
}
