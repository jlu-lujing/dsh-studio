...
const GLOBAL_SOURCE = 'dsh-studio-webui.global'
/** 当前选中「预设/自定义」主题的 override 层 source（不改官方 preference）。 */
const ACTIVE_SOURCE = 'dsh-studio-webui.active'
...
async function fetchHostThemes(): Promise<{
  ...
  try {
    const res = await fetch('/dsh-studio-webui/themes')
...
function postHostDelete(id: string): void {
  try {
    fetch('/dsh-studio-webui/themes/delete', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
