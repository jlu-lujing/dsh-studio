...
import { WorktreeController } from '../src/client/controller.ts'
...
test('controller：list/attribution 走 host 路由并带 cwd', async () => {
  ...
  const fetchImpl = fakeFetch({
    'GET /dsh-kit-worktree/worktrees': (url) => {
    ...
    'GET /dsh-kit-worktree/attribution': () => ({
      ok: true,
      status: 200,
...
test('controller：createAndBind 先 POST 建 git worktree，再注册+开会话', async () => {
  ...
  let posted = null
  const fetchImpl = fakeFetch({
    'POST /dsh-kit-worktree/worktrees': (url, opts) => {
...
test('controller：host 错误透传为 Error', async () => {
  const fetchImpl = fakeFetch({
    'GET /dsh-kit-worktree/attribution': { ok: false, status: 500, json: async () => ({ error: 'git failed', detail: 'fatal: x' }) },
  })
  const controller = new WorktreeController(makeWorkspaces([]), fetchImpl)
